...
class Interpreter():
	...
	def eval(self, statement ):
		if type(statement) == list:
			...
			if type(self.eval(statement.first)) == int and self.eval(statement.first) != 0:
			...
			self.func_list[statement.first.value] = {"args": statement.second, "body": statement.third}
		elif statement.id == "let":
			self.var_list[statement.first.first.value] = self.eval(statement.first.second) 	
